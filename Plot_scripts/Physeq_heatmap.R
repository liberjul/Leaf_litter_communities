...
phy_sample_table <- data.frame(Substrate = map_wo_negs["Substrate",], # Convert some variable to sample data table
...
phy_sample_table <- phy_sample_table %>%
  mutate(Name = paste(Site,
                      unlist(strsplit(Plant_species, " "))[c(T, F)],
                      sep = "-"))
rownames(phy_sample_table) <- sample_names(physeq)
phy_sample_table <- sample_data(phy_sample_table)
phy_sample_table
physeq1 <- merge_phyloseq(physeq, phy_sample_table) # Merge sample data to physeq object
physeq1
ps_heatmap <- plot_heatmap(physeq1, "NMDS", "bray", "Name") # Make heatmap
...
ps_heatmap$scales$scales[[1]]$name <- "Sample"
ps_heatmap
